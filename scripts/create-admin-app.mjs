...
export function createAdminApp(options = {}) {
  const adminToken = options.adminToken || process.env.ADMIN_TOKEN || 'artiada2025'
  ...
  app.post('/api/auth/check', (req, res) => {
    ...
    if (token === adminToken) res.json({ ok: true })
    else res.status(401).json({ error: 'Неверный пароль' })
  })

  app.use('/api', auth(adminToken))
